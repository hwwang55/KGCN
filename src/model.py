...
class KGCN:
    ...
    def _build_model(self, n_user, n_entity, n_relation):
        ...
        self.params.append(self.entity_emb_matrix)
        self.params.append(self.relation_emb_matrix)
        ...
        # {[batch_size], [batch_size, n_neighbor], ..., [batch_size, n_neighbor^2], [batch_size, n_neighbor^n_iter]}
        entities, relations = self._get_neighbors(self.item_indices)

        # [batch_size, dim]
        self.item_embeddings, aggregators = self._aggregate(entities, relations)

    def _get_neighbors(self, seeds):
        seeds = tf.expand_dims(seeds, axis=1)
        entities = [seeds]
        relations = [[0]]  # [0] is only a placeholder
    ...
    def _aggregate(self, entities, relations):
        ...
        for i in range(self.n_iter):
            ...
            for hop in range(self.n_iter - i):
                ...
                vector = aggregator(self_vectors=entity_vectors[hop],
                                    ...
                                    neighbor_relations=tf.reshape(relation_vectors[hop + 1], shape),
